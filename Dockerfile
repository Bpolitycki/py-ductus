...
RUN python -m venv /opt/py-ductus-env
ENV PATH /opt/py-ductus-env/bin:$PATH
ENV VIRTUAL_ENV /opt/py-ductus-env

# Set the working directory.
WORKDIR /workspaces/py-ductus/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/py-ductus/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/py_ductus/ && touch src/py_ductus/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/py-ductus/
